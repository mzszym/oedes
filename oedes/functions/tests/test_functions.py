...
def test_scharfgum():
    ...
    assert np.allclose(B(x), b_x, rtol=1e-6, atol=1e-300)
    assert np.allclose(Aux1(x), aux1_x, rtol=1e-6, atol=1e-300)
    assert np.allclose(Aux2(x), aux2_x, rtol=1e-6, atol=1e-300)
    assert np.allclose(ScharfetterGummelFlux(
        1., 2., 1., 100., 1., False), 200.)
    flux, midc, dc = ScharfetterGummelFlux(1., 2., 1., 100., 1., True)
    assert np.allclose(flux, 200.)
    assert np.allclose(midc, 2.)
    assert np.allclose(dc, -1.92874985e-20, atol=0.)
...
def check_gdos_impl(impl):
    ...
    b = impl.b(a, I)
    I1 = impl.I(a, b)
    assert np.allclose(I, I1, atol=0., rtol=1e-12)
    g = gdos.diffusion_enhancement(impl.a_max, I, impl)
    assert np.allclose(g[0], 1., atol=0., rtol=1e-5)
    ...
    assert np.allclose(gdos.diffusion_enhancement(
...
def test_egdm():
    ...
    assert np.allclose(egdm.g1(6., c), g1_6, rtol=1e-6, atol=0.)
    En = np.array([0., 0.5, 1., 1.5, 2.0])
    g2_6 = np.array([1., 1.69015385, 6.54410149, 40.54260014,
                     320.61218011])
    assert np.allclose(egdm.g2(6., En), g2_6, rtol=1e-6, atol=0.)
    g3_6 = np.array(
        [1.15279545, 1.31057169, 1.6093657, 2.36265441, 3.57370846])
    assert np.allclose(egdm.g3(6., c), g3_6, rtol=1e-2, atol=0.)
    Vt = physics.ThermalVoltage(300.)
    assert np.allclose(egdm.nsigma_inverse(6., Vt), 0.1551119460699098)
    assert np.allclose(egdm.nsigma(0.1551119460699098, Vt),
                       6., rtol=1e-6, atol=0.)
